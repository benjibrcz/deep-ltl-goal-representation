...
def main():
    ...
    p.add_argument('--out',         type=str, default='probe_dynamics.png')
    args = p.parse_args()
    ...
    fig.suptitle(f"Formula: {formula}", fontsize=16)
    ...
    for i, s in enumerate(snaps):
        ...
        for z_idx, ((zx,zy), c_idx) in enumerate(zip(true_zpos, true_zcol)):
            ...
            ax.add_patch(patches.Circle((pred_zx, pred_zy), 0.3,
                facecolor="none",
    ...
    if args.gif:
        ...
        def init():
            ...
            return [traj_line] + pred_patches

        def update(i):
            t = i * args.gif_dt
            if t >= len(feats): return [traj_line] + pred_patches
            ...
            for z_idx, patch in enumerate(pred_patches):
                if patch:
                    pred_c_idx = (clfs[z_idx].predict(x)[0]
                                  if clfs[z_idx]
                                  else int(np.unique(Ycol[:,z_idx])[0]))
                    patch.set_edgecolor(CMAP_RGB[pred_c_idx])
                    patch.set_center(ppos[z_idx])

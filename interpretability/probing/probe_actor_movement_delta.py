#!/usr/bin/env python3
"""
Actor Movement Delta Probing

This script probes the Actor network for future movement delta prediction:
- INPUT: Combined embedding (env_embedding + ltl_embedding) going INTO actor
- OUTPUT: Action logits/probabilities coming OUT of actor
- CRITICAL: Actor has access to BOTH spatial info (env_net) AND goal info (ltl_net)
- This should reveal TRUE INTENTIONS vs just trajectory extrapolation

Usage: python interpretability/probing/probe_actor_movement_delta.py --probe-type input --k-steps 1
"""
...
class ActorMovementDeltaProbe:
    """Probe the Actor network for movement delta prediction."""
    ...
    def collect_data(self, world_ids, n_rollouts_per_world=10, max_steps=200):
        """Collect Actor activations and future movement deltas."""
        print(f"Collecting ACTOR {self.probe_type.upper()} data from {len(world_ids)} worlds for {self.k_steps}-step movement delta")
        ...
        for world_id in world_ids:
            ...
            for rollout_id in trange(n_rollouts_per_world, desc=f"Rollouts for world {world_id}"):
                ...
                def capture_actor_input_output(module, input, output):
                    if self.probe_type == 'input':
                        ...
                        if hasattr(output, 'dist'):
                            # MixedDistribution - access underlying distribution
                            if hasattr(output.dist, 'logits'):
                                ...
                                # Fallback - try to get parameters
                ...
                # Hook the Actor module (not env_net!)
                ...
                for step_id in range(max_steps):
                    ...
                    # Store the captured activation from Actor
                ...
                for i in range(valid_samples):
                    if i >= len(trajectory_activations):
                        break
    ...
    def train_and_evaluate_probes(self, splits):
        """Train and evaluate Actor movement delta prediction probes."""
        results = {}
...
def main():
    parser = argparse.ArgumentParser(description='Actor Movement Delta Probing')
    ...
    print("=== ACTOR MOVEMENT DELTA PROBING ===")
    ...
    if args.probe_type == 'input':
        ...
        print("💡 This contains BOTH spatial info AND goal information!")
    else:
        print("📤 Probing: Action logits/probabilities coming OUT of actor")
        print("💡 This contains the final action decisions!")
    ...
    # Create probe
    probe = ActorMovementDeltaProbe(model, args.probe_type, args.k_steps, args.n_components)
    ...
    print("\nTraining and evaluating Actor movement delta probes...")
    ...
    for split_name, metrics in results.items():
        csv_data.append({
            'timestamp': timestamp,
            'probe_type': f'actor_{args.probe_type}',
    ...
    csv_path = f"{args.output_dir}/actor_movement_delta_{args.probe_type}_k{args.k_steps}_{timestamp}.csv"
    df.to_csv(csv_path, index=False)
    
    print(f"Results saved to: {csv_path}")
    
    print(f"\n=== ACTOR MOVEMENT DELTA PROBE SUMMARY (k={args.k_steps}) ===")
    for split_name, metrics in results.items():

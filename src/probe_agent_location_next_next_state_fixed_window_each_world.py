#!/usr/bin/env python3
"""
This script trains a linear probe to predict the agent's next-next position (at t+2) from a fixed window of previous activations ending at t, for each world.

Key differences from previous scripts:
- Predicts position at t+2 from activation at t (next-next state)
- Uses proper train/test split by rollout (not by individual samples)
- Tests generalization to unseen rollouts

Usage: python src/probe_agent_location_next_next_state_fixed_window_each_world.py --layer env_net.mlp.2 --window-size 10 --world-id -1
"""
...
def collect_hidden_and_next_next_positions_multiple_worlds(model, layer_name, sampler, n_rollouts=10, max_steps=200, world_ids=None):
    ...
    for world_id in world_ids:
        ...
        for rollout_idx in trange(n_rollouts, desc=f"Rollouts for world {world_id}"):
            ...
            # For next-next-state prediction: align feats[t] with positions[t+2]
            if len(positions) > 2 and len(feats) > 2:
                feats = feats[:-2]      # activations at t
                positions = positions[2:]  # positions at t+2
            h.remove()
            if len(feats) > len(positions):
...
def create_fixed_window_features_next_next_state(activations, positions, window_size=10):
...
def train_fixed_window_probes_by_world_next_next_state(world_data, window_size=10):
    world_probes = {}
    world_results = {}
    print(f"\nTraining fixed-window next-next-state probes for each world (window size = {window_size})...")
    for world_id in sorted(world_data.keys()):
        ...
        X_train_world, Y_train_world, _ = create_fixed_window_features_next_next_state(train_activations, train_positions, window_size=window_size)
        
        # Create test data
        test_activations = [activations[i] for i in test_indices]
        test_positions = [positions[i] for i in test_indices]
        X_test_world, Y_test_world, _ = create_fixed_window_features_next_next_state(test_activations, test_positions, window_size=window_size)
...
def main():
    ...
    world_data = collect_hidden_and_next_next_positions_multiple_worlds(
    ...
    if not world_data:
        ...
        return
    world_probes, world_results = train_fixed_window_probes_by_world_next_next_state(world_data, window_size=args.window_size)
    if world_results:
        ...
        ax1.set_title(f'Next-Next-State Fixed Window Prediction R² by World (window size = {args.window_size})')
        ...
        ax2.set_title('Next-Next-State Fixed Window Prediction MSE by World')
        ax2.grid(True, alpha=0.3)
        for bar, score in zip(bars2, mse_scores):
            height = bar.get_height()
            ax2.text(bar.get_x() + bar.get_width()/2., height, f'{score:.4f}', ha='center', va='bottom')
        plt.tight_layout()
        plot_file = args.out or f'next_next_state_fixed_window_probes_world{args.world_id}_window{args.window_size}.png'
        plt.savefig(plot_file, dpi=150)
        print(f"\nSaved next-next-state fixed window probe analysis to {plot_file}")
        df = pd.DataFrame([
            {
                'world_id': world_id,
        ...
        csv_file = f'next_next_state_fixed_window_probes_world{args.world_id}_window{args.window_size}.csv'
        df.to_csv(csv_file, index=False)
        print(f"Saved next-next-state fixed window results to {csv_file}")
        ...
        for world_id in sorted(world_results.keys()):
            ...
            for plot_idx, rollout_idx in enumerate(test_indices):
                ...
                if len(predicted_positions) > 0:
                    ...
                    # Plot actual next-next positions (blue)
                    ax.plot(rollout_positions[args.window_size:, 0], rollout_positions[args.window_size:, 1], '-o', color='blue', markersize=3, alpha=0.7, label='True Next-Next')
                    # Plot predicted next-next positions (red)
                    pred_pos_array = np.array(predicted_positions)
                    ax.plot(pred_pos_array[:, 0], pred_pos_array[:, 1], '-o', color='red', markersize=4, alpha=0.8, label='Predicted Next-Next')
                    ...
                    ax.legend()
            plt.tight_layout()
            trajectory_file = f'next_next_state_fixed_window_trajectories_world{world_id}_window{args.window_size}.png'
            plt.savefig(trajectory_file, dpi=150)
            print(f"  Saved next-next-state trajectory plot for world {world_id} to {trajectory_file}")
            plt.close()
        ...
        print("No next-next-state fixed window probes could be trained (insufficient data).")

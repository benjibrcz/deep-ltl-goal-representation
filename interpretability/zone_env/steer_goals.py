...
class GoalSteerer:
    ...
    def get_steered_hidden(self, hidden_state, current_goal):
        ...
        print(f"STEERING: {current_goal} -> {target_goal}")
        self.steer_count += 1
        
        return steered_hidden
    
    def hook_fn(self, mod, inp, out):
        """Hook function that edits hidden states during forward pass"""
                ...
                if len(goal_set) == 1:
                    ...
                    if len(true_props) == 1:
                        ...
                        if self.should_steer(current_goal):
                            ...
                            self.original_hidden = h_n.detach().clone()
                            
                            # Get steered hidden state
                            steered_hidden = self.get_steered_hidden(
                                h_n.detach().squeeze(0).squeeze(0).cpu().numpy(), 
                                current_goal
                            )
                            
                            # Replace the hidden state
                            new_h_n = torch.tensor(steered_hidden, dtype=h_n.dtype, device=h_n.device)
                            new_h_n = new_h_n.unsqueeze(0).unsqueeze(0)  # Add batch and seq dims
                            
                            # Replace the output - return as tuple
                            if len(out) > 2:
                                return (out[0], new_h_n, out[2])
                            else:
                                return (out[0], new_h_n)
        
        # Return original output if no steering
        return out

def train_goal_probes(model, env, sampler_fn):
    """Train probes to predict blue vs green goals"""
    print("Training goal probes...")
    
    # Collect data for probing
    feats = []
    labels = []
    
    # Hook to collect features
    ...
    if hasattr(model.ltl_net, 'rnn') and model.ltl_net.rnn is not None:
        handle = model.ltl_net.rnn.register_forward_hook(collect_hook)
    ...
    print(f"Blue probe accuracy: {blue_probe.score(X, y):.2%}")
    print(f"Green probe accuracy: {green_probe.score(X, 1-y):.2%}")
...
def main():
    ...
    # Train goal probes
    blue_probe, green_probe = train_goal_probes(model, ENV, sampler_fn)
    if blue_probe is None:
        print("Failed to train probes, exiting")
        return
    
    # 2) Create agent and steerer
    search = ExhaustiveSearch(model, set(['blue', 'green']), num_loops=2)
    agent = Agent(model, search=search, propositions=set(['blue', 'green']), verbose=False)
    # Store agent reference for hook access
    steerer = GoalSteerer(model, blue_probe, green_probe)
    steerer.agent = agent  # Store agent reference
    
    # 3) Hook into LTL network for steering
    handle = None
    if hasattr(model.ltl_net, 'rnn') and model.ltl_net.rnn is not None:
        handle = model.ltl_net.rnn.register_forward_hook(steerer.hook_fn)
    
    # 4) Run steered rollout
    print(f"\nRunning steered rollout (steering frequency: {STEER_FREQ})...")
    rollout_env = make_env(ENV, sampler_fn, sequence=False, render_mode=None)
    ret = rollout_env.reset(seed=SEED)
    obs, info = ret if isinstance(ret, tuple) and len(ret) == 2 else (ret, {})
    agent.reset()
    ...
    for step in trange(MAX_STEPS, desc="Steered rollout"):
        # Get agent position
    ...
    for color, positions_list in zone_positions.items():
        for pos in positions_list:
            circle = patches.Circle(pos, 0.1, color=color, alpha=0.3)
            plt.gca().add_patch(circle)
    
    plt.xlabel('X Position')
    plt.ylabel('Y Position')
    plt.title(f'Agent Trajectory with Goal Steering (Frequency: {STEER_FREQ})')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.axis('equal')
    
    # Save plot
    plt.savefig('steered_trajectory.png', dpi=150, bbox_inches='tight')
    plt.show()
    
    print(f"\nTrajectory saved as 'steered_trajectory.png'")
    print(f"Steering interventions: {steerer.steer_count}")

...
def is_vec(x):
    try:
        a=np.asarray(x); return a.ndim==1 and a.size>0
    except:
        return False
...
def fit_eval_ridge(X,Y,tr_idx,te_idx,alphas):
...
def fit_eval_mlp(X,Y,tr_idx,te_idx,hidden,alpha,max_iter,seed):
    Xsc = StandardScaler().fit(X[tr_idx])
    Ysc = StandardScaler().fit(Y[tr_idx])
    Xtr,Xte = Xsc.transform(X[tr_idx]), Xsc.transform(X[te_idx])
    Ytr,Yte = Ysc.transform(Y[tr_idx]), Ysc.transform(Y[te_idx])
    mlp = MLPRegressor(hidden_layer_sizes=hidden, alpha=alpha, max_iter=max_iter, random_state=seed)
    mlp.fit(Xtr,Ytr)
    return float(mlp.score(Xte,Yte))
...
def main():
    ...
    for subset_name, mask in [
        ...
        for name,(ua,ug) in [
            ...
            r2_lin = fit_eval_ridge(X,Y,tr_local,te_local,alphas)
            msg = f"  {name:12s} | Ridge R^2={r2_lin:.3f}"
            if args.mlp:
                r2_mlp = fit_eval_mlp(X,Y,tr_local,te_local,hidden, args.mlp_alpha, args.mlp_max_iter, args.seed)
                msg += f" | MLP R^2={r2_mlp:.3f}"
            print(msg)

...
class PolicyNetworkSteerer:
    ...
    def get_steered_hidden(self, hidden_state, current_goal):
        ...
        print(f"STEERING POLICY: {current_goal} -> {target_goal}")
        self.steer_count += 1
        
        return steered_hidden
    
    def hook_fn(self, mod, inp, out):
        """Hook function that edits policy network hidden states during forward pass"""
        # Get the output from policy network
...
def find_policy_layers(model):
    """Find all possible policy network layers to hook into"""
    policy_layers = []
    
    print("Searching for policy network layers...")
    
    # Try to find policy network components
    if hasattr(model, 'policy'):
        print(f"Found policy network: {type(model.policy)}")
        
        # Check for different possible layer structures
        for name, module in model.policy.named_modules():
            print(f"  Policy layer: {name} - {type(module)}")
            if hasattr(module, 'weight') and module.weight is not None:
                policy_layers.append((name, module))
                print(f"    -> Added as hookable layer (shape: {module.weight.shape})")
    
    # Also check for actor network
    if hasattr(model, 'actor'):
        print(f"Found actor network: {type(model.actor)}")
        for name, module in model.actor.named_modules():
            print(f"  Actor layer: {name} - {type(module)}")
            if hasattr(module, 'weight') and module.weight is not None:
                policy_layers.append((name, module))
                print(f"    -> Added as hookable layer (shape: {module.weight.shape})")
...
def train_policy_goal_probes(model, env, sampler_fn):
    """Train probes to predict blue vs green goals from policy network"""
    print("Training policy network goal probes...")
    
    # Find policy layers
    policy_layers = find_policy_layers(model)
    
    if not policy_layers:
        print("No policy layers found to hook into!")
        return None, None, None
    
    # Collect data for probing from the first available layer
    layer_name, layer_module = policy_layers[0]
    print(f"Using policy layer: {layer_name}")
    
    feats = []
    labels = []
    
    # Hook to collect features from policy network
    ...
    handle = layer_module.register_forward_hook(collect_hook)
    ...
    print(f"Policy blue probe accuracy: {blue_probe.score(X, y):.2%}")
    print(f"Policy green probe accuracy: {green_probe.score(X, 1-y):.2%}")
    
    return blue_probe, green_probe, layer_name
...
def main():
    ...
    # 1) Load model and train policy network probes
    ...
    # Train policy network goal probes
    blue_probe, green_probe, layer_name = train_policy_goal_probes(model, ENV, sampler_fn)
    if blue_probe is None:
        print("Failed to train policy network probes, exiting")
        return
    ...
    steerer = PolicyNetworkSteerer(model, blue_probe, green_probe)
    steerer.agent = agent
    
    # 3) Hook into policy network for steering
    policy_layers = find_policy_layers(model)
    if not policy_layers:
        print("No policy layers found for steering")
        return
    
    # Use the same layer we used for probing
    layer_name, layer_module = policy_layers[0]
    handle = layer_module.register_forward_hook(steerer.hook_fn)
    print(f"Registered policy network steering hook on layer: {layer_name}")
    
    # 4) Run steered rollout
    print(f"\nRunning policy network steered rollout (steering frequency: {STEER_FREQ})...")
    rollout_env = make_env(ENV, sampler_fn, sequence=False, render_mode=None)
    ret = rollout_env.reset(seed=SEED)
    obs, info = ret if isinstance(ret, tuple) and len(ret) == 2 else (ret, {})
    agent.reset()
    ...
    for step in trange(MAX_STEPS, desc="Policy network steered rollout"):
        # Get agent position
    ...
    # 5) Analyze results
    print(f"\nPolicy Network Steering Results:")
    ...
    plt.title(f'Agent Trajectory with Policy Network Steering (Frequency: {STEER_FREQ})')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.axis('equal')
    
    # Save plot
    plt.savefig('policy_steered_trajectory.png', dpi=150, bbox_inches='tight')
    plt.show()
    
    print(f"\nTrajectory saved as 'policy_steered_trajectory.png'")
    print(f"Policy network steering interventions: {steerer.steer_count}")

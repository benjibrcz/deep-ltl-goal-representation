#!/usr/bin/env python3
"""
Fit a linear controlled transition model for the GRU hidden state:

    h_{t+1} ~= A h_t + B u_t + c

where u_t concatenates (optionally) the action a_t and/or a one-hot goal indicator.
Evaluates 1-step test R^2 and free k-step rollouts (MSE and cosine).

Inputs
------
- A rollouts parquet from log_rollouts.py (must contain at least: chain_id, h)
  and ideally also: step/t, a (action), goal_text (e.g., "FG green").

Outputs
-------
- <out_dir>/transition_linear.npz  (A, B, c, feature meta)
- <out_dir>/scalers.pkl            (sklearn scalers for X and y, pickle)
- <out_dir>/metrics.json           (R^2, rollout MSE/cosine per k)
- console logs with a concise summary
...
def rollout_free(A: np.ndarray, B: np.ndarray, c: np.ndarray,
                 h0: np.ndarray, U_seq: np.ndarray, k: int) -> np.ndarray:
    """
    Free rollout for k steps: returns predicted h_{t+1..t+k}
...
def main():
    ...
    print(f"One-step held-out R^2: {r2_1step:.3f}")
    ...
    #             = (Ystd * W / Xstd) X + (Ymean - Ystd * W * Xmean / Xstd + Ystd * b)
    ...
    # Evaluate free k-step rollouts on test chains
    # Build per-row control vector U_t = concat(action onehot pieces we used)
    if B.shape[1] > 0:
        U_all = X[:, Hdim:].astype(np.float32)
    else:
        U_all = np.zeros((len(X), 0), dtype=np.float32)

    # attach indices back to pairs for grouping
    ...
    for g, gdf in pairs.groupby("chain_id"):
        ...
        # contiguous rows correspond to consecutive steps by construction
        seqs.append(rows)
    ...
    for k in k_eval:
        mses, coss = [], []
        for rows in seqs:
            # use only if at least k steps inside this chain in the test set
            # Require that all rows used are from the test split to avoid leakage
            rows_k = [r for r in rows if r in te_rows]
            if len(rows_k) < (k+1):
                continue
            # slide windows inside the test rows
            for i in range(len(rows_k) - k):
                r0 = rows_k[i]
                # ground-truth
                h0 = X[r0, :Hdim]
                Y_true = Y[r0+1 : r0+1+k]   # k rows of h_{t+1..t+k}
                if Y_true.shape[0] < k:
                    break
                U_seq = U_all[r0+1 : r0+1+k] if U_all.shape[1] > 0 else np.zeros((k,0), dtype=np.float32)
                Y_hat = rollout_free(A, B, c, h0, U_seq, k)
                mses.append(float(np.mean((Y_hat - Y_true)**2)))
                coss.append(float(np.mean([cosine(Y_hat[j], Y_true[j]) for j in range(k)])))
        if mses:
            roll_metrics[k]["mse"] = float(np.mean(mses))
            roll_metrics[k]["cosine"] = float(np.mean(coss))
        else:
            roll_metrics[k]["mse"] = None
            roll_metrics[k]["cosine"] = None
    ...
    # Console summary
    print("\nSummary:")
    print(f"  One-step R^2 (test): {r2_1step:.3f}")
    for k in k_eval:
        m = roll_metrics[k]
        print(f"  Free rollout k={k:<2d}:  MSE={m['mse']!s:<12}  mean cos={m['cosine']!s}")

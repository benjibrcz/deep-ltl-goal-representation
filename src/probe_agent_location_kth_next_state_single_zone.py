...
def collect_hidden_and_kth_next_positions_single_zone(model, layer_name, sampler, n_trials=10, max_steps=200, k=1, world_idx=0):
    ...
    # Load a single world file as reference
    ...
    reference_zone_positions = dict(env.zone_positions)  # Store reference zone positions
    
    # Find world files with similar zone layouts
    similar_worlds = []
    for i in range(50):  # Check first 50 worlds
        test_file = f"{world_dir_path}/world_info_{i}.pkl"
        if os.path.exists(test_file):
            try:
                env.load_world_info(test_file)
                current_zone_positions = dict(env.zone_positions)
                # Check if zone positions are similar (same number of zones and similar positions)
                if len(current_zone_positions) == len(reference_zone_positions):
                    similar_worlds.append(i)
                    if len(similar_worlds) >= n_trials:
                        break
            except:
                continue
    
    print(f"Found {len(similar_worlds)} worlds with similar zone layouts")
    
    successful_trials = 0
    for i, world_idx in enumerate(similar_worlds[:n_trials]):
        try:
            world_file = f"{world_dir_path}/world_info_{world_idx}.pkl"
            env.load_world_info(world_file)
            ...
            if len(positions) > k:
                ...
                zone_poss.append(reference_zone_positions)  # Use reference zone positions for all trials
                world_trajs.append(np.stack(positions))
                successful_trials += 1
        except Exception as e:
            print(f"Failed to collect data from world {world_idx}: {e}")
            continue
...
def main():
    ...
    # ── collect data ─────────────────────────────────────────────────────────────
    X_all, Y_all, zone_poss, world_trajs, step_idx_all = collect_hidden_and_kth_next_positions_single_zone(
    ...
    print(f"\nAgent k-th next location probe results (k={k}, single zone, world {args.world_idx}):")
    ...
    for i, (X_seq, Y_seq, zone_pos, true_traj) in enumerate(zip(X_all, Y_all, zone_poss, world_trajs)):
        ...
        ax.legend()
    plt.tight_layout()
    out_file = args.out or f'agent_location_kth_next_state_single_zone_k{k}_world{args.world_idx}.png'
    plt.savefig(out_file, dpi=150)
    print(f"\nSaved agent k-th next location single zone trajectory probe plot to {out_file}")
    ...
    if len(per_step_results) > 0:
        ...
        plot_file = f'agent_location_kth_next_state_single_zone_r2_by_step_k{k}_world{args.world_idx}.png'
        ...
        plt.title(f'Single Zone Probe R^2 by Step (k={k}, world {args.world_idx})')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(plot_file, dpi=150)
        print(f"Saved R^2 by step plot to {plot_file}")
        csv_file = f'agent_location_kth_next_state_single_zone_r2_by_step_k{k}_world{args.world_idx}.csv'

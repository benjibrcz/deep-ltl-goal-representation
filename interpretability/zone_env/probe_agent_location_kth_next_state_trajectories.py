...
def main():
    ...
    # ── collect data ─────────────────────────────────────────────────────────────
    X_all, Y_all, zone_poss, world_trajs, step_idx_all = collect_hidden_and_kth_next_positions_per_world(
    ...
    print(f"\nAgent k-th next location probe results (k={k}):")
    ...
    for i, (X_seq, Y_seq, zone_pos, true_traj) in enumerate(zip(X_all, Y_all, zone_poss, world_trajs)):
        ...
        ax.legend()
    plt.tight_layout()
    out_file = args.out or f'agent_location_kth_next_state_trajectories_k{k}.png'
    ...
    plot_file = 'agent_location_kth_next_state_trajectories_r2_by_step.png'
    ...
    plt.title(f'Global Probe R^2 by Step (k={k})')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(plot_file, dpi=150)
    print(f"Saved R^2 by step plot to {plot_file}")
    csv_file = 'agent_location_kth_next_state_trajectories_r2_by_step.csv'

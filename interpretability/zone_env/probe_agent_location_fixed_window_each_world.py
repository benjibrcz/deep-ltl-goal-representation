...
EXP        = "big_test"
SEED       = 0
N_ROLLOUTS = 1  # Number of different starting positions per world
...
def collect_hidden_and_positions_multiple_worlds(model, layer_name, sampler, n_rollouts=10, max_steps=200, world_ids=None):
    """
    Collect data from multiple worlds for fixed window prediction.
    ...
    for world_id in world_ids:
        ...
        for rollout_idx in trange(n_rollouts, desc=f"Rollouts for world {world_id}"):
            ...
            # Remove the first position (no corresponding activation) and last activation (no corresponding position)
            if len(positions) > 0 and len(feats) > 0:
                positions = positions[1:]  # Remove first position (no activation yet)
                feats = feats[:-1]  # Remove last activation (no position after)
            h.remove()
            if len(feats) > len(positions):
...
def create_fixed_window_features(activations, positions, window_size=10):
    """
    For each step >= window_size, use activations from previous `window_size` steps to predict position at current step.
    """
...
def train_fixed_window_probes_by_world(world_data, window_size=10):
    ...
    for world_id in sorted(world_data.keys()):
        ...
        X_train_world, Y_train_world, _ = create_fixed_window_features(train_activations, train_positions, window_size=window_size)
        
        # Create test data
        test_activations = [activations[i] for i in test_indices]
        test_positions = [positions[i] for i in test_indices]
        X_test_world, Y_test_world, _ = create_fixed_window_features(test_activations, test_positions, window_size=window_size)
...
def main():
    ...
    world_probes, world_results = train_fixed_window_probes_by_world(world_data, window_size=args.window_size)
    ...
    if world_results:
        ...
        print(f"Saved fixed window results to {csv_file}")
        
        # ── create trajectory plots for each world ───────────────────────────────
        ...
        for world_id in sorted(world_results.keys()):
            ...
            for plot_idx, rollout_idx in enumerate(test_indices):
                ...
                # Generate predictions for this rollout
                rollout_length = len(rollout_positions)
                predicted_positions = []
                ...
                if len(predicted_positions) > 0:
                    ...
                    # Plot predicted positions (starting from window_size)
                    ...
                    ax.plot(rollout_positions[0, 0], rollout_positions[0, 1], 'go', 
                           markersize=8, label='Start')
                    ax.plot(rollout_positions[-1, 0], rollout_positions[-1, 1], 'ro', 
            ...
            plt.tight_layout()
            trajectory_file = f'fixed_window_trajectories_world{world_id}_window{args.window_size}.png'
            plt.savefig(trajectory_file, dpi=150)
            print(f"  Saved trajectory plot for world {world_id} to {trajectory_file}")
            plt.close()

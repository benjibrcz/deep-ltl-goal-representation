...
def train_and_evaluate_nonlinear_movement_probes(splits, k_steps=1, probe_type='mlp', hidden_layers=(64, 32), max_iter=500):
    """
    Train and evaluate nonlinear movement vector probes for each type of generalization.
    """
    results = {}
    ...
    for split_name, split_data in splits.items():
        ...
        # Create nonlinear probe based on type
        if probe_type == 'mlp':
            probe = MLPRegressor(
        ...
        # Train probe with error handling for MLPs
        try:
            probe.fit(X_train, y_train)
        except Exception as e:
            print(f"  {split_name}: Training failed: {e}")
            continue
...
def create_nonlinear_movement_vector_visualization(results, k_steps, probe_type, save_path=None):
    """
    Create comprehensive visualization of nonlinear movement vector predictions.
    """
    if not results:
    ...
    fig.suptitle(f'{k_steps}-Step Movement Vectors: Nonlinear {probe_type.upper()} Probe Analysis', 
    ...
    ax1.set_title(f'{k_steps}-Step Nonlinear {probe_type.upper()} Performance')
    ax1.set_xticks(x)
    ...
    ax5.set_title(f'{k_steps}-Step Movement Magnitude ({probe_type.upper()})')
    ax5.legend()
    ...
    stats_text = f"""{k_steps}-STEP NONLINEAR MOVEMENT VECTORS
...
Nonlinear Probe Analysis:
  - Predicting {k_steps}-step movement vectors
  - {probe_type.upper()} can capture nonlinear spatial patterns
  - Tests complex planning capabilities
"""
        ...
        if probe_type == 'mlp':
            probe_info = sample_result['probe']
            stats_text += f"""
MLP Architecture:
  Hidden layers: {probe_info.hidden_layer_sizes}
  Iterations: {probe_info.n_iter_}
  Training loss: {probe_info.loss_:.6f}
"""
...
"""
    ...
    if save_path:
        ...
        print(f"Saved nonlinear movement vector analysis to {save_path}")
...
def main():
    ...
    p.add_argument('--probe-type',  type=str, default='mlp', choices=['mlp', 'rf'], 
                   help='Type of nonlinear probe (mlp=Multi-layer Perceptron, rf=Random Forest)')
    p.add_argument('--hidden-layers', nargs='+', type=int, default=[64, 32], 
                   help='Hidden layer sizes for MLP (e.g., --hidden-layers 128 64 32)')
    p.add_argument('--max-iter',    type=int, default=500, help='Maximum iterations for MLP training')
    ...
    print(f"Predicting {args.k_steps}-step movement vectors with {args.probe_type.upper()} probe")
    if args.probe_type == 'mlp':
        print(f"MLP architecture: {tuple(args.hidden_layers)} -> 2")
    ...
    # ── train and evaluate nonlinear probes ────────────────────────────────────
    print(f"\nTraining {args.k_steps}-step nonlinear {args.probe_type.upper()} movement vector probes...")
    results = train_and_evaluate_nonlinear_movement_probes(
        splits, k_steps=args.k_steps, probe_type=args.probe_type, 
        hidden_layers=tuple(args.hidden_layers), max_iter=args.max_iter)
    ...
    if results:
        ...
        plot_file = args.out or f'{results_dir}/nonlinear_movement_vectors_{args.k_steps}step_{args.probe_type}_{layer_name}_{timestamp}.png'
        
        create_nonlinear_movement_vector_visualization(results, args.k_steps, args.probe_type, save_path=plot_file)
        
        # Print summary
        print(f"\n{args.k_steps}-Step Nonlinear {args.probe_type.upper()} Movement Vector Results:")
        ...
        print("No nonlinear probes could be trained (insufficient data).")

...
def main():
    ...
    p.add_argument('--n-trajectories', type=int, default=4, help='Number of trajectories to show')
    p.add_argument('--world-ids', nargs='+', type=int, default=[5, 6, 7, 8], help='World IDs to test on')
    args = p.parse_args()
    ...
    n_trajectories = min(args.n_trajectories, len(args.world_ids))
    cols = min(2, n_trajectories)
    rows = (n_trajectories + cols - 1) // cols
    ...
    fig.suptitle(f'Multi-Layer Delta Predictions Comparison', fontsize=16)
    
    for traj_idx in range(n_trajectories):
        if traj_idx >= len(args.world_ids):
            break
            
        world_id = args.world_ids[traj_idx]
        seed = SEED + (traj_idx + 1) * 2000
        
        row = traj_idx // cols
        col = traj_idx % cols
        
        try:
            # Collect trajectory with multi-layer activations
            traj_data = collect_multi_layer_trajectory(
                model, args.layers, sampler, world_id, seed, max_steps=35)
            
            if len(traj_data['positions']) < 3:
                current_ax = axes[row, col] if rows > 1 else axes[col]
                current_ax.text(0.5, 0.5, 'Insufficient data', 
                                  ha='center', va='center', transform=current_ax.transAxes)
                current_ax.set_title(f'World {world_id}')
                continue
            
            # Set up the plot
            ax = plt.subplot(rows, cols, traj_idx + 1, axes_class=FancyAxes, 
            ...
            # Add error text box
            if layer_errors:
                error_text = "Final Errors:\n" + "\n".join([
                    f"{layer.split('.')[-1]}: {error:.2f}" 
                    for layer, error in sorted(layer_errors.items())
                ])
                ax.text(0.02, 0.98, error_text, transform=ax.transAxes, va='top', fontsize=8,
                       bbox=dict(boxstyle="round,pad=0.4", facecolor="white", alpha=0.9))
            
            ax.set_title(f'World {world_id} - Multi-Layer Comparison', fontsize=12)
            
        except Exception as e:
            current_ax = axes[row, col] if rows > 1 else axes[col]
            current_ax.text(0.5, 0.5, f'Error: {str(e)[:30]}...', 
                              ha='center', va='center', transform=current_ax.transAxes)
            current_ax.set_title(f'World {world_id}')
    
    # Hide unused subplots
    for traj_idx in range(n_trajectories, rows * cols):
        row = traj_idx // cols
        col = traj_idx % cols
        if rows > 1:
    ...
    fig.legend(handles=legend_elements, loc='lower center', 
              bbox_to_anchor=(0.5, -0.05), ncol=min(4, len(legend_elements)))
    ...
    plot_file = f'{results_dir}/multi_layer_deltas_{layer_names_str}_{timestamp}.png'
